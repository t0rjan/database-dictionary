    ...
    public static List<TableInfo> getBuildPdfTableData(Connection connection) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                String sqlColumn = "SELECT A.attname AS 列名,concat_ws('',t.typname,SUBSTRING(format_type(A.atttypid,A.atttypmod) from '\\(.*\\)')),(CASE WHEN A.attlen> 0 THEN A.attlen ELSE A.atttypmod-4 END) AS 长度,A.attnotnull AS 是否可为空,pg_get_expr(d.adbin,d.adrelid) AS 默认值,col_description (A.attrelid,A.attnum) AS 备注 FROM pg_class C,pg_attribute A LEFT JOIN (SELECT A.attname,ad.adbin,ad.adrelid FROM pg_class C,pg_attribute A,pg_attrdef ad WHERE relname='" + tableName + "' AND ad.adrelid=C.oid AND adnum=A.attnum AND attrelid=C.oid) AS d ON A.attname=d.attname  LEFT JOIN pg_type t on A.atttypid = t.oid WHERE C.relname='" + tableName + "' AND A.attrelid=C.oid AND A.attnum> 0 AND position('pg.dropped' in A.attname) = 0;";
